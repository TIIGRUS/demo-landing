...
## Правила подключения стилей

`src/styles/index.css` — единая точка входа и реестр уровня переопределения. Правила подключения:

- **Элементы (`__*`)** импортируются внутри корневого файла блока (`block.css`) до его собственных правил:

```css
/* menu.css */
@import url('./__item/menu__item.css');
@import url('./__item/_active/menu__item_active.css');

.menu { … }
```

- **Модификаторы блока (`_*`)** импортируются в `index.css` сразу после своего блока — так порядок каскада предсказуем и виден в одном месте:

```css
/* index.css */
@import url('blocks/menu/menu.css');
@import url('blocks/menu/_size/menu_size_mobile.css');
@import url('blocks/menu/_size/menu_size_tablet.css');
@import url('blocks/menu/_place/menu_place_footer.css');
```

> **Почему не в `block.css`?** CSS-спецификация требует, чтобы `@import` стоял до любых правил в файле. Если поместить `@import ./_mod/…` в `block.css`, он будет загружен _до_ базовых стилей блока, и базовый стиль перекроет модификатор по каскаду. Хранение модификаторов в `index.css` — осознанное архитектурное решение, соответствующее БЭМ Nested.

**Правило ревью:** новые элементы добавляются в импорты `block.css`, новые модификаторы — в `index.css` после своего блока.
